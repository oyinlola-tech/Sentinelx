...
async def list_rules(principal: Viewer, platform: PlatformDep) -> dict[str, Any]:
    return {
        ...
        "load_problems": platform.rules.load_problems,
    }
...
async def validate(
    body: RuleDefinitionRequest, principal: Analyst, platform: PlatformDep
) -> dict[str, Any]:
...
@router.post(
    "/rules/test",
    summary="Run a rule in isolation against a scenario, a PCAP, or its embedded tests",
)
async def test(body: RuleTestRequest, principal: Analyst, platform: PlatformDep) -> dict[str, Any]:
    if body.scenario and body.scenario not in SCENARIOS:
        raise HTTPException(
            status_code=422, detail=f"unknown scenario; available: {', '.join(sorted(SCENARIOS))}"
        )
    pcap = None
    if body.pcap_path:
...
async def create_rule(
    body: RuleDefinitionRequest, principal: Admin, request: Request, platform: PlatformDep
) -> dict[str, Any]:
    return await platform.rules.create(
        body.definition, actor=principal.username, source=_source(request)
    )
...
async def update_rule(
    rule_id: str,
    body: RuleDefinitionRequest,
    principal: Admin,
    request: Request,
    platform: PlatformDep,
) -> dict[str, Any]:
    try:
        return await platform.rules.update(
            rule_id, body.definition, actor=principal.username, source=_source(request)
        )
...
async def set_enabled(
    ...
    try:
        return await platform.rules.set_enabled(
            rule_id, body.enabled, actor=principal.username, source=_source(request)
        )
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="rule not found") from exc
...
async def delete_rule(
    rule_id: str, principal: Admin, request: Request, platform: PlatformDep
) -> None:
    try:
...
async def toggle_detector(
    name: str, body: RuleEnabledRequest, principal: Admin, request: Request, platform: PlatformDep
) -> dict[str, Any]:
    pipeline, _, _, _ = platform.require()
    if name.startswith("rule:"):
        raise HTTPException(
            status_code=422, detail="use /rules/{rule_id}/enabled for rule detectors"
        )
    ...
    await platform.config.update(
        "detection",
        {"disabled_detectors": sorted(disabled)},
