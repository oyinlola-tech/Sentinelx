...
def max_rule_window(settings: Settings) -> float:
    ...
    return max(
        detection.port_scan_window_seconds, detection.brute_force_window_seconds,
        detection.connection_rate_window_seconds, detection.icmp_flood_window_seconds,
        detection.dns_window_seconds, detection.http_flood_window_seconds,
    )
...
class RuleService:
    ...
    def parse(self, definition: str, *, source: str = "api") -> Rule:
        ...
        rules, problems = parse_rule_document(document, source=source, max_window_seconds=self.max_window)
        if problems:
    ...
    async def sync_files(self) -> None:
        ...
        async with self.database.session() as session:
            ...
            for rule in result.rules:
                ...
                if record is not None and record.origin == "api":
                    self.load_problems.append(f"{rule.source}: rule id '{rule.id}' is already used by an API rule; file rule skipped")
                    continue
                await repo.upsert(
                    rule.id, name=rule.name, definition=rule_to_yaml(rule), origin="file", source_path=rule.source,
        ...
        log.info("rules_synced", files=len(result.files), rules=len(result.rules), problems=len(result.problems))
    ...
    async def create(self, definition: str, *, actor: str, source: str) -> dict[str, Any]:
        ...
        async with self.database.session() as session:
            ...
            await repo.upsert(rule.id, name=rule.name, definition=rule_to_yaml(rule), origin="api",
                              enabled=rule.enabled, updated_by=actor)
        await self._changed("CREATE_RULE", rule.id, actor, source, {"condition": rule.condition, "action": rule.action.value})
        return await self.get(rule.id) or {}

    async def update(self, rule_id: str, definition: str, *, actor: str, source: str) -> dict[str, Any]:
        rule = self.parse(definition)
        async with self.database.session() as session:
            repo = RuleRepository(session)
            ...
            if record.origin == "file":
                raise RuleValidationError(rule.name, [f"'{rule_id}' is defined in {record.source_path}; edit the file instead"])
            if rule.id != rule_id:
                raise RuleValidationError(rule.name, ["renaming a rule changes its id; create a new rule instead"])
            await repo.upsert(rule_id, name=rule.name, definition=rule_to_yaml(rule), updated_by=actor)
        await self._changed("UPDATE_RULE", rule_id, actor, source, {"condition": rule.condition, "action": rule.action.value})
        return await self.get(rule_id) or {}

    async def set_enabled(self, rule_id: str, enabled: bool, *, actor: str, source: str) -> dict[str, Any]:
        async with self.database.session() as session:
            repo = RuleRepository(session)
            if await repo.get(rule_id) is None:
                raise KeyError(rule_id)
            await repo.upsert(rule_id, enabled=enabled, updated_by=actor)
        await self._changed("ENABLE_RULE" if enabled else "DISABLE_RULE", rule_id, actor, source, {})
        return await self.get(rule_id) or {}
    ...
    async def delete(self, rule_id: str, *, actor: str, source: str) -> None:
        async with self.database.session() as session:
            ...
            if record is None:
                raise KeyError(rule_id)
            if record.origin == "file":
                raise RuleValidationError(record.name, [f"'{rule_id}' is defined in {record.source_path}; delete it there or disable it"])
            await repo.delete(rule_id)
        await self._changed("DELETE_RULE", rule_id, actor, source, {})

    async def test(self, definition: str, *, scenario: str | None = None, pcap_path: Path | None = None) -> dict[str, Any]:
        rule = self.parse(definition)
        ...
        if scenario is not None:
            ...
            result = await asyncio.to_thread(run_rule_on_frames, rule, frames, self.settings.detection)
            return {"target": scenario, **result.as_dict()}
        outcomes = await asyncio.to_thread(run_rule_tests, rule, self.settings.detection)
        return {"target": "embedded tests", "tests": [o.as_dict() for o in outcomes],
                "passed": all(o.passed for o in outcomes), "count": len(outcomes)}

    async def _changed(self, action: str, rule_id: str, actor: str, source: str, details: dict[str, Any]) -> None:
        active = await self.apply()
        await self.audit.record(actor=actor, action=action, target=rule_id, source=source, details=details)
        await self.bus.publish(EventType.RULE_CHANGED, {"rule_id": rule_id, "action": action, "active_rules": active})
    ...
    def fields() -> list[dict[str, str]]:
        return [{"name": spec.name, "kind": spec.kind.value, "description": spec.description} for spec in FIELDS.values()]
