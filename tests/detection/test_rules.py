...
class TestParser:
    ...
    def test_values_lists_strings_booleans(self) -> None:
        node = parse_condition('destination_port in [22, 2222] and http_host endswith ".test" and handshake_complete == false')
    ...
    @pytest.mark.parametrize("text", [
        ...
        "protocol == TCP and", "destination_port in []", "destination_port in [1, [2]]",
        "protocol == TCP; import os", "__import__('os').system('id')", "protocol == `id`",
        "handshake_complete", "not not not handshake_complete",
    ])
...
class TestSemantics:
    ...
    def test_all_problems_reported_together(self) -> None:
        assert len(validate_semantics(parse_condition("nope == 1 and protocol > 3 and ttl contains x"))) >= 3
...
class TestEvaluation:
    ...
    def test_in_network(self) -> None:
        assert self.evaluate('source_ip in_network ["10.0.0.0/8", "2001:db8::/32"]', {"source_ip": "2001:db8::5"})[0]
    ...
    def test_evidence_excludes_negated_matches(self) -> None:
        result, fields = self.evaluate("ttl > 1 and not protocol == udp", {"ttl": 64, "protocol": "tcp"})
        assert result and fields == ["ttl"]


class TestRuleValidation:
    def rule(self, **overrides: object) -> Rule:
        base: dict[str, object] = {"name": "Test Rule", "condition": "protocol == TCP and short_sessions >= 20", "within": "60s"}
        return Rule.model_validate({**base, **overrides})

    def test_block_without_count_threshold_is_refused(self) -> None:
        problems = validate_rule(self.rule(condition="protocol == TCP", action="block_ip"), max_window_seconds=60)
    ...
    def test_unselective_preventive_conditions_are_refused(self, condition: str) -> None:
        problems = validate_rule(self.rule(condition=condition, action="block_ip"), max_window_seconds=60)
        assert any("count threshold" in p for p in problems)

    def test_selective_preventive_rule_is_accepted(self) -> None:
        rule = self.rule(condition="(short_sessions >= 20 or failed_attempts > 50) and destination_port == 22",
                         action="temporary_block", duration=900)
        assert validate_rule(rule, max_window_seconds=60) == []

    def test_alert_rule_needs_no_threshold(self) -> None:
        assert validate_rule(self.rule(condition="tls_is_legacy_version == true", action="log"), max_window_seconds=60) == []

    def test_window_longer_than_feature_history_is_refused(self) -> None:
        assert any("exceeds" in p for p in validate_rule(self.rule(within="10m"), max_window_seconds=60))

    def test_temporary_block_requires_duration(self) -> None:
        assert any("duration" in p for p in validate_rule(self.rule(action="temporary_block"), max_window_seconds=60))

    def test_rules_cannot_unblock(self) -> None:
        with pytest.raises(ValueError, match="unblock"):
            self.rule(action="unblock_ip")

    @pytest.mark.parametrize(("value", "seconds"), [("30s", 30), ("5m", 300), ("1h", 3600), ("500ms", 0.5), (45, 45)])
    ...
    def test_unknown_keys_are_rejected(self) -> None:
        rules, problems = parse_rule_document({"rules": [{"name": "abc", "condition": "ttl > 1", "exec": "id"}]},
                                              source="x", max_window_seconds=60)
...
class TestLoading:
    ...
    def test_duplicate_ids_and_invalid_yaml(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "d.yml").write_text("!!python/object/apply:os.system ['id']\n", encoding="utf-8")
        result = load_rules(tmp_path, max_window_seconds=60)
        assert len(result.rules) == 1
        assert any("duplicates" in p for p in result.problems)
        assert sum("not valid YAML" in p for p in result.problems) == 2  # safe_load refuses python tags


class TestRuleDetection:
    def test_rule_detection_carries_rule_name_and_explicit_evidence(self) -> None:
        rule = Rule(name="Brute", condition="destination_port == 22 and short_sessions >= 20", within=60)
    ...
    async def test_run_on_pcap(self, tmp_path: Path) -> None:
        ...
        rule = Rule(name="Tunnel", condition="dns_label_length >= 40 and dns_query_count >= 50", within=30)
