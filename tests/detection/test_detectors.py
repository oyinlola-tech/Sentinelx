...
class TestTcpPortScan:
    def test_evidence_matches_observed_traffic(self, run_detection: Run) -> None:
        detections = [
            d
            for d in run_detection(get_scenario("tcp_port_scan", ports=150).frames)
            if d.detector == "tcp_port_scan"
        ]
        final = detections[-1]
    ...
    def test_escalates_to_critical_as_scan_grows(self, run_detection: Run) -> None:
        detections = [
            d
            for d in run_detection(get_scenario("tcp_port_scan", ports=220).frames)
            if d.detector == "tcp_port_scan"
        ]
    ...
    def test_busy_client_completing_handshakes_is_not_a_scan(self, run_detection: Run) -> None:
        ...
        for port in range(8000, 8060):  # 60 ports, but every handshake completes
            ...
            packets += [
                ...
                (
                    build_tcp(
                        "10.0.0.5", "10.0.0.9", 40000 + port, port, flags="PA", payload=b"x" * 40
                    ),
                    now + 0.003,
                ),
            ]
    ...
    def test_exactly_at_threshold_fires(self, run_detection: Run) -> None:
        threshold = DetectionSettings().port_scan_unique_ports
        assert "tcp_port_scan" in detectors_fired(
            run_detection(get_scenario("tcp_port_scan", ports=threshold).frames)
        )
    ...
    def test_slow_scan_outside_window_is_not_detected(self, run_detection: Run) -> None:
        ...
        packets = [
            (
                build_tcp("203.0.113.9", "10.0.0.1", 50000, port, flags="S"),
                BASE_TIME + i * (settings.port_scan_window_seconds / 3),
            )
...
class TestHorizontalScan:
    ...
    def test_sensitive_port_sweep_is_critical(self, run_detection: Run) -> None:
        detections = [
            d
            for d in run_detection(get_scenario("horizontal_scan", port=445).frames)
            if d.detector == "horizontal_scan"
        ]
...
class TestUdpScan:
    def test_dns_resolver_replying_to_many_clients_is_not_a_scan(self, run_detection: Run) -> None:
        packets = [
            (
                build_udp("10.0.0.53", f"10.0.1.{i % 250 + 1}", 53, 30000 + i, b"\x00" * 20),
                BASE_TIME + i * 0.01,
            )
            for i in range(300)
        ]
...
class TestBruteForce:
    def test_evidence_counts_attempts(self, run_detection: Run) -> None:
        detections = [
            d
            for d in run_detection(get_scenario("ssh_brute_force", attempts=30).frames)
            if d.detector == "ssh_brute_force"
        ]
        assert detections and detections[0].destination_port == 22
        assert (
            detections[0].evidence_dict()["failed_attempts"]
            >= DetectionSettings().brute_force_attempts
        )
...
class TestFloods:
    def test_syn_flood_recommends_rate_limit(self, run_detection: Run) -> None:
        detections = [
            d for d in run_detection(get_scenario("syn_flood").frames) if d.detector == "syn_flood"
        ]
    ...
    def test_ordinary_ping_is_silent(self, run_detection: Run) -> None:
        packets = [
            (build_icmp("10.0.0.1", "10.0.0.2", sequence=i), BASE_TIME + i) for i in range(60)
        ]
        assert run_detection(frames_from(packets)) == []
    ...
    def test_http_flood_positive_and_moderate_rate_negative(self, run_detection: Run) -> None:
        ...
        moderate = [
            (build_http_request("10.0.0.3", "10.0.0.100", 40000 + i), BASE_TIME + i * 0.1)
            for i in range(200)
        ]
    ...
    def test_connection_rate_threshold_edge(self, run_detection: Run) -> None:
        settings = DetectionSettings(
            connection_rate_threshold=50, port_scan_unique_ports=10_000, syn_flood_threshold=10_000
        )
        just_under = [
            (build_tcp("10.9.9.9", "10.0.0.1", 30000 + i, 443, flags="S"), BASE_TIME + i * 0.01)
            for i in range(49)
        ]
...
class TestDns:
    def test_tunnel_evidence_names_parent_domain(self, run_detection: Run) -> None:
        detections = [
            d
            for d in run_detection(get_scenario("dns_tunneling").frames)
            if d.detector == "dns_anomaly"
        ]
        assert detections[0].evidence_dict()["parent_domain"] == "example.test"

    def test_long_names_spread_across_many_domains_are_not_a_tunnel(
        self, run_detection: Run
    ) -> None:
        packets = [
            (
                build_dns_query("10.0.0.66", "10.0.0.1", f"{'a' * 55}{i}.cdn{i}.net"),
                BASE_TIME + i * 0.2,
            )
            for i in range(60)
        ]
    ...
    def test_normal_repeated_lookups_are_silent(self, run_detection: Run) -> None:
        ...
        packets = [
            (build_dns_query("10.0.0.66", "10.0.0.1", names[i % 3]), BASE_TIME + i * 0.1)
            for i in range(250)
        ]
        assert run_detection(frames_from(packets)) == []
...
class TestPolicyDetectors:
    ...
    def test_outbound_to_denylisted_destination_is_attributed_to_listed_host(
        self, run_detection: Run
    ) -> None:
        settings = DetectionSettings(denylist_networks=["198.51.100.99/32"])
        detections = run_detection(
            frames_from(
                [(build_tcp("10.0.0.5", "198.51.100.99", 5555, 443, flags="S"), BASE_TIME)]
            ),
            settings,
        )
    ...
    def test_empty_denylist_never_fires(self, run_detection: Run) -> None:
        assert (
            run_detection(
                frames_from([(build_tcp("203.0.113.66", "10.0.0.1", 1, 2, flags="S"), BASE_TIME)])
            )
            == []
        )
    ...
    def test_illegal_flag_combinations(self, run_detection: Run, flags: str) -> None:
        packets = [
            (build_tcp("198.51.100.8", "10.0.0.1", 40000, 80 + i, flags=flags), BASE_TIME + i)
            for i in range(3)
        ]
    ...
    def test_single_illegal_packet_is_tolerated(self, run_detection: Run) -> None:
        assert (
            run_detection(
                frames_from([(build_tcp("198.51.100.8", "10.0.0.1", 1, 80, flags=""), BASE_TIME)])
            )
            == []
        )
...
class TestDocumentedEvasions:
    ...
    def test_slow_port_scan_evades_window_detector(self, run_detection: Run) -> None:
        assert "tcp_port_scan" not in detectors_fired(
            run_detection(get_scenario("slow_port_scan").frames)
        )

    def test_low_rate_brute_force_evades_threshold(self, run_detection: Run) -> None:
        assert not {"ssh_brute_force", "auth_brute_force"} & detectors_fired(
            run_detection(get_scenario("low_rate_brute_force").frames)
        )
    ...
    def test_port_scan_window_setting_is_honoured(self, run_detection: Run) -> None:
        ...
        assert "tcp_port_scan" not in detectors_fired(
            run_detection(frames, DetectionSettings(port_scan_window_seconds=15))
        )
