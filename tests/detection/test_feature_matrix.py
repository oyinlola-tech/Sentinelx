...
def tcp(src: str, dst: str, sport: int, dport: int, spec: str, ts: float, length: int = 60) -> PacketEvent:
    return PacketEvent(
        timestamp=ts,
        src_ip=src,
        dst_ip=dst,
...
def udp(src: str, dst: str, sport: int, dport: int, ts: float, *, length: int = 80, metadata: dict[str, Any] | None = None) -> PacketEvent:
    return PacketEvent(
        timestamp=ts,
        src_ip=src,
        dst_ip=dst,
...
def dns_query(src: str, name: str, ts: float, *, label: int | None = None, entropy: float = 2.0) -> PacketEvent:
...
class TestSourceProfile:
    def test_unique_ports_and_destinations(self) -> None:
        ...
        for i in range(30):
            ...
            extractor.process(tcp(src, f"10.0.0.{i % 10}", 50000 + i, 1000 + i % 3, "S", T0 + i * 0.1))
    ...
    def test_packet_rate_and_size_statistics_are_exact(self) -> None:
        ...
        assert f["packet_size_mean"] == round(mean, 2) and f["packet_size_stddev"] == round(stddev, 2)
    ...
    def test_replies_drive_syn_ack_and_refusal_ratios(self) -> None:
        ...
        for port in range(1, 11):
            ...
            extractor.process(tcp(target, scanner, port, 40000 + port, reply, T0 + port * 0.01 + 0.001))
    ...
    def test_dns_responses_and_malformed_dns_metadata_are_ignored(self) -> None:
        extractor = FeatureExtractor()
        src = "10.0.0.67"
        extractor.process(udp(src, "10.0.0.1", 40000, 53, T0, metadata={"dns": {"is_response": True, "query_name": "a.b"}}))
        extractor.process(udp(src, "10.0.0.1", 40000, 53, T0 + 1, metadata={"dns": "not-a-dict"}))
        extractor.process(udp(src, "10.0.0.1", 40000, 53, T0 + 2, metadata={"dns": {"query_name": None}}))
        extractor.process(udp(src, "10.0.0.1", 40000, 53, T0 + 3, metadata={"dns": {"query_name": "ok.test", "max_label_length": None, "name_entropy": None}}))
    ...
    def test_http_request_frequency_and_paths(self) -> None:
        ...
        response = PacketEvent(
            timestamp=T0 + 5, src_ip=src, dst_ip="10.0.0.80", protocol=Protocol.TCP, length=60,
            src_port=50000, dst_port=80, tcp_flags=flags("PA"),
    ...
    def test_fresh_profile_snapshot_is_all_zero(self) -> None:
        ...
        for key in ("packet_count", "unique_dst_ports", "syn_count", "dns_query_count", "short_sessions"):
...
class TestFlows:
    def test_both_directions_share_one_flow_and_handshake_completes(self) -> None:
        ...
        assert (flow.initiator_ip, flow.initiator_port, flow.responder_ip, flow.responder_port) == (a, 40000, b, 443)
...
class TestGlobalStats:
    def test_protocol_distribution_is_exact(self) -> None:
        ...
        packets = [
            ...
            PacketEvent(timestamp=T0 + 4, src_ip="10.0.0.1", dst_ip="10.0.0.2", protocol=Protocol.ICMPV6, length=70),
            PacketEvent(timestamp=T0 + 5, src_ip="10.0.0.1", dst_ip="10.0.0.9", protocol=Protocol.ARP, length=42),
            PacketEvent(timestamp=T0 + 6, src_ip="10.0.0.1", dst_ip="10.0.0.2", protocol=Protocol.IPV4, length=50),
            PacketEvent(timestamp=T0 + 7, src_ip="fe80::1", dst_ip="fe80::2", protocol=Protocol.IPV6, length=50),
        ]
        for packet in packets:
            extractor.process(packet)
        stats = extractor.stats
        assert (stats.packets, stats.tcp, stats.udp, stats.icmp, stats.arp, stats.other) == (8, 2, 1, 2, 1, 2)
        assert stats.bytes_total == sum(p.length for p in packets)
        assert stats.protocol_distribution() == {"tcp": 0.25, "udp": 0.125, "icmp": 0.25, "arp": 0.125, "other": 0.25}
    ...
    def test_empty_extractor(self) -> None:
        ...
        assert GlobalStats().protocol_distribution() == dict.fromkeys(("tcp", "udp", "icmp", "arp", "other"), 0.0)
...
class TestBoundedState:
    def test_200k_packets_from_50k_sources_stays_within_caps(self) -> None:
        ...
        for s in range(sources):
            ...
            for p in range(4):  # 200k packets in total
                ts += 0.0002
                extractor.process(
                    PacketEvent(timestamp=ts, src_ip=ip, dst_ip="192.168.0.1", protocol=Protocol.TCP,
                                length=60, src_port=1024 + p, dst_port=80, tcp_flags=syn)
                )
    ...
    def test_memory_does_not_grow_with_distinct_source_count(self) -> None:
        def footprint(sources: int) -> int:
            ...
            tracemalloc.start()
            for s in range(sources):
                extractor.process(icmp(f"172.16.{s >> 8 & 255}.{s & 255}", "10.0.0.1", T0 + s * 0.001))
