...
async def run_pipeline_with_persistence(
    database: Database, scenario: str, replay_id: str | None = None
) -> EventPersister:
    settings = Settings()
    bus = EventBus()
    persister = EventPersister(database, bus, settings)
    pipeline = Pipeline(
        settings, bus=bus, firewall=MemoryFirewall(), audit=AuditService(database, bus).sink
    )
...
async def test_replay_data_is_isolated_from_live_views(database: Database) -> None:
    ...
    async with database.session() as session:
        ...
        assert (
            await AnalyticsRepository(session).summary(datetime.now(UTC) - timedelta(days=3650))
        )["detections"] == 0
...
async def test_detection_filters_and_like_escaping(database: Database) -> None:
    ...
    async with database.session() as session:
        for index, (severity, title) in enumerate(
            [("high", "100% match"), ("low", "100 matches"), ("critical", "scan")]
        ):
            session.add(
                DetectionRecord(
                    ...
                    risk_score=50 + index * 20,
                )
            )
    async with database.session() as session:
        repo = DetectionRepository(session)
        assert (
            await repo.page(DetectionFilter(search="100%"))
        ).total == 1  # % is literal, not a wildcard
        ...
        assert (await repo.page(DetectionFilter(source_ip="203.0.113.1"))).items[
            0
        ].title == "100 matches"
    ...
    async with database.session() as session:
        ...
        timeline = await AnalyticsRepository(session).timeline(
            now - timedelta(days=1), 60, database.dialect
        )
        ...
        assert (
            24 <= len(timeline) <= 26
            and sum(1 for bucket in timeline if bucket["total"] == 0) >= 20
        )
...
async def test_audit_redacts_secrets_and_orders_newest_first(database: Database) -> None:
    ...
    await audit.record(
        ...
        details={"jwt_secret": "s3cr3t", "field": "dry_run"},
    )
    await audit.record(
        actor="admin", action="block_ip", target="203.0.113.5", reason="scan", source="dashboard"
    )
    async with database.session() as session:
        ...
        assert (await AuditRepository(session).page(action="block_ip")).items[
            0
        ].target == "203.0.113.5"


async def test_retention_purges_old_data_but_keeps_open_incidents_and_recent_audit(
    database: Database,
) -> None:
...
class TestSharedState:
    async def test_degraded_mode_when_redis_unreachable(self) -> None:
        ...
        results = [
            await state.hit("login", "1.2.3.4", limit=3, window_seconds=60) for _ in range(5)
        ]
    ...
    async def test_required_redis_refuses_to_start(self) -> None:
        ...
        with pytest.raises(StorageError):
            await SharedState(
                StorageSettings(redis_url="redis://127.0.0.1:1/0", redis_required=True)
            ).connect()

    async def test_sliding_window_expires(self) -> None:
        clock = [1000.0]
        state = SharedState(
            StorageSettings(redis_url="redis://127.0.0.1:1/0"), clock=lambda: clock[0]
        )
        await state.connect()
        for _ in range(3):
