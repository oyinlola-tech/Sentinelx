...
import { ago, endpoint, humanise, timestamp } from "@/lib/format";
import type { Detection } from "@/lib/types";

export function DetectionTable({ detections, emptyTitle = "No detections", emptyBody, showStatus = true, compact = false, captureTime = false }: { detections: Detection[]; emptyTitle?: string; emptyBody?: string; showStatus?: boolean; compact?: boolean; captureTime?: boolean }) {
  // A replay's detections carry the capture's own timestamps, so "1035d ago" would be
  // true but useless: those views ask for the capture time instead.
          ...
          {detections.map((detection) => {
            ...
            return (
              <tr key={detection.detection_id} data-href={href} onClick={() => router.push(href)}>
                <td className="whitespace-nowrap"><Mono className="text-mist" >{captureTime ? timestamp(detection.timestamp) : ago(detection.timestamp)}</Mono></td>
