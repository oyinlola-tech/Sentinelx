...
- **Silent database partitions.** A stopped, crashed or unreachable PostgreSQL server is reported at once (API 503, health `error`) and security events are buffered and written when it returns. A server that accepts connections but stops answering (a network partition that drops packets) can hold requests on already-open connections until the operating system gives up on them, because closing such a connection waits for the peer.
