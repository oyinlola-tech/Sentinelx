...
- **Silent database partitions.** A stopped, crashed or unreachable PostgreSQL server is reported at once (API 503, health `error`) and security events are buffered and written when it returns. A server that accepts connections but stops answering (a paused server, or a partition that drops packets) is reported after `STORAGE__SESSION_TIMEOUT_SECONDS` (default 10 s): the request answers 503, the connection is discarded without waiting for the server, and buffered events are written once it answers again.
