...
| 8 | `local_addresses_unknown` | `protect_management_addresses` is true (default) and this host's interface addresses cannot be enumerated. The guard fails closed rather than risk blocking the host itself. Refused interface statistics alone (state, MTU, counters) do not trigger this; only unreadable addresses do. | `could not list this host's addresses (<error>); refusing to block` |
...
`auto` picks the first usable backend for the platform (nftables, then iptables on Linux; pf on macOS and BSD; Windows Firewall on Windows). A backend is usable when its tool is installed, `firewall_privilege()` grants the privilege (on Linux: root, or `CAP_NET_ADMIN` that can be passed to child processes; on macOS: root; on Windows: an elevated process), and a read-only functional probe succeeds: `nft list tables`, `iptables -w -S INPUT` or `pfctl -s info`, run through `CommandRunner` with a 5-second timeout. The probe never changes the firewall. It catches a kernel without nf_tables, or a sandbox or user namespace that refuses netlink, and the report then carries the tool's own error, for example `nft is installed but 'nft list tables' failed: Operation not permitted (you must be root)`. Probe results are cached for 30 seconds per backend, like the capability report. Windows Firewall has no functional probe, because its adapter has not been run on a real host. If none is usable, `auto` resolves to `null` and the reason is kept for health and the capability report.
...
The runner can prefix `sudo -n`, but no setting enables this. SentinelX therefore needs root, `CAP_NET_ADMIN` (Linux) or an elevated process (Windows) to change the firewall. `sentinelx doctor` and `sentinelx capabilities` report whether the configured backend's tool is installed, whether firewall privileges are available, and whether the tool's read-only probe works.
...
- **Replacing.** A block or rate limit first lists the chain's existing rules for that network, inserts the new rule, and only then deletes the old ones. The address is never unblocked in between, and a failed insert leaves the old rule in force. A block therefore replaces an existing rate limit for the same network, and the reverse. If deleting an old rule fails, the adapter keeps the kernel and the engine's registry in agreement:
  - If the old rule is already gone (removed by someone else meanwhile), the replacement counts as done.
  - Otherwise the new rule is withdrawn and the action fails with the real error, ending in `the new rule was withdrawn and the existing rule stays in force`. The kernel is left exactly as it was before the attempt.
  - If the withdrawal fails too, the new rule is in force, so the action is reported as executed, and the leftover rule is logged at error level as `iptables_replaced_rule_left_in_place`. Unblocking the network removes it.
- **Listing** uses `iptables -S SENTINELX`. Where a network has more than one rule, the first one in the chain is reported, because it is the one iptables applies. A missing chain means nothing is blocked; any other failure raises `FirewallError`.
