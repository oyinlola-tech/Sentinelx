...
| `database` | FAIL if the database cannot be reached (remedy `check DATABASE_URL`). WARN, without connecting, when `DATABASE_URL` names a SQLite file that does not exist yet: `database file does not exist yet (<path>); it is created and migrated when SentinelX starts` (the migrations check is then skipped). `doctor` connects without preparing the schema, so it never creates or migrates the database it inspects. |
...
| `STORAGE__SESSION_TIMEOUT_SECONDS` | | `10` | no | Seconds one unit of database work may take in total: an API request's database session (waiting for a connection, its statements and the commit). Past it the connection is discarded without waiting on the server and the API answers 503. Batches of security events use the persister's own 60-second write timeout instead, so a slow batch is not abandoned (events stay buffered and are retried when a write does fail), and retention purges use ten times the statement timeout (0 < value ≤ 3600). |
