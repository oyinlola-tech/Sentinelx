...
- **Redis outages weaken shared limits.** Without Redis, rate limits, login throttling, per-address lockout counters, WebSocket tickets and the access-token revocation list fall back to process memory. They then apply per process, and entries are lost if that process restarts before Redis returns (revocations still held in memory are written back on reconnect). The per-user session cut-off set by sign-out, password changes and resets is stored in the database and is not affected. Set `storage.redis_required=true` if that is unacceptable.
