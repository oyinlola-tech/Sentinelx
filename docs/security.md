...
- A generated bootstrap administrator password is never printed or logged, since console output is kept by `docker logs` and the systemd journal. It is written to a `0600` file in a private directory owned by the service account (`API__BOOTSTRAP_PASSWORD_FILE`), the console shows only that path, and the file is deleted when the password is changed at the required first sign-in. Symlinked, foreign-owned or shared directories are refused.
- The dashboard receives tokens in `HttpOnly`, `SameSite=Strict` cookies, and state-changing requests must carry a double-submit CSRF token in `X-CSRF-Token`. Scripts use bearer tokens instead. Protected `/api/v1` requests are authenticated before the body is read, so an anonymous caller always gets 401, never a validation error that reveals how a route parses its input. `POST /auth/refresh` uses the refresh cookie only together with the `X-SentinelX-Client: dashboard` header, which a page on another site cannot add to a cross-site request without a CORS preflight; without it the request is refused with 403, so the cookie alone never rotates a session.
